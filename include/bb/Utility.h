...
void TrainDataNormalize(TrainData<T> &td)
{
    ...
    std::vector<T>  x_max((std::size_t)x_size, (T)1.0);
    std::vector<T>  x_min((std::size_t)x_size, (T)0.0);
    std::vector<T>  t_max((std::size_t)t_size, (T)1.0);
    std::vector<T>  t_min((std::size_t)t_size, (T)0.0);

    // 集計
    for (auto& x : td.x_train) {
        for (std::size_t i = 0; i < (std::size_t)x_size; ++i) {
    ...
    for (auto& t : td.t_train) {
        for (std::size_t i = 0; i < (std::size_t)t_size; ++i) {
