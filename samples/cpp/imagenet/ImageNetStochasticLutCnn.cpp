...
#include "bb/Runner.h"
#include "bb/ImageNet.h"
#include "bb/ExportVerilog.h"


void ImageNetStochasticLutCnn(int epoch_size, int mini_batch_size, int test_modulation_size, bool binary_mode, bool file_read)
{
    std::string net_name       = "ImageNetStochasticLutCnn";
    std::string velilog_path   = "../../verilog/imagenet/";
    std::string velilog_module = "ImageNetLutCnn";

  // load cifar-10 data
#ifdef _DEBUG
    auto td = bb::LoadImageNet<>::Load(1);
    std::cout << "!!! debug mode !!!" << std::endl;
#else
    auto td = bb::LoadImageNet<>::Load();
#endif
...

    {
...

        {
            ...
            // write test image
//          bb::WriteTestDataImage<float>(velilog_path + "imagenet_test_160x120.ppm", 160, 120, td);
//          bb::WriteTestDataImage<float>(velilog_path + "imagenet_test_640x480.ppm", 640, 480, td);
        }
    }
}
